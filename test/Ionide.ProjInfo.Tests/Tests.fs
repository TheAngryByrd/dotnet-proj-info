...
type UnknownBuildFailure(data: GraphBuildResult) =
    inherit Exception("Build failed but no exception was filled out on GraphBuildResult.")
...
type BuildManagerSession(?bm: BuildManager, ?buildParameters: BuildParameters) =
    ...
    do bm.BeginBuild(buildParameters)

    /// <summary>Submits a graph build request to the current build and starts it asynchonously.</summary>
    /// <param name="graphRequest">GraphBuildRequestData encapsulates all of the data needed to submit a graph build request.</param>
    /// <returns></returns>
    member _.BuildAsync(graphRequest: GraphBuildRequestData) = async {
...
let projectInstanceFactoryNeedName tfmSelector (projectPath: string) (xml: Dictionary<string, string>) (collection: ProjectCollection) =
...
type ProjectGraphLoader =

    static member Evalutate(entryProjectFile: string, ?projectCollection: ProjectCollection, ?projectInstanceFactory) =
        ProjectGraphLoader.Evalutate([ ProjectGraphEntryPoint entryProjectFile ], ?projectCollection = projectCollection, ?projectInstanceFactory = projectInstanceFactory)

    static member Evalutate(entryProjectFile: ProjectGraphEntryPoint seq, ?projectCollection: ProjectCollection, ?projectInstanceFactory) =
        let pc = defaultArg projectCollection ProjectCollection.GlobalProjectCollection
        let projectInstanceFactory = defaultArg projectInstanceFactory (projectInstanceFactoryNeedName selectFirstTfm)
        ProjectGraph(entryProjectFile, pc, projectInstanceFactory)

    static member Execution(session: BuildManagerSession, graph: ProjectGraph, ?targetsToBuild, ?flags) = async {
        ...
        let gbr = GraphBuildRequestData(projectGraph = graph, targetsToBuild = targetsToBuild, hostServices = null, flags = flags)

        return! session.BuildAsync(gbr)
    }
    ...
    static member Parse(graphBuildResult: GraphBuildResult) =
        ...
        |> ProjectGraphLoader.Parse

    static member Parse(projectInstances: ProjectInstance seq) =
        projectInstances
        |> Seq.toArray
        |> Array.Parallel.map (fun p -> ProjectLoader.getLoadedProjectInfo p.FullPath [] (ProjectLoader.LoadedProject p))
    ...
    ftestList "buildManagerSessionTests" [
        ...
        |> withLog
            ...
            (fun logger fs ->
                ...
                let graph = ProjectGraphLoader.Evalutate(path, pc)

                // Execution
                let bp = BuildParameters(ProjectLoadSettings = ProjectLoadSettings.FailOnUnresolvedSdk, EnableNodeReuse = true)
                use bm = new BuildManagerSession(buildParameters = bp)

                let result =
                    ProjectGraphLoader.Execution(bm, graph)
                    |> Async.RunSynchronously

                // Parse
                let projectsAfterBuild =
                    ProjectGraphLoader.Parse result
                    |> Array.map (
                        function
                        | Ok v -> v
        ...
        testCase
        |> withLog
            "Concurrency - issue execution with some graph"
            (fun logger fs ->
                let path =
                    [ @"C:\Users\jimmy\Repositories\public\TheAngryByrd\FAKE\Fake.sln" ]
                    |> List.map ProjectGraphEntryPoint
                ...
                File.WriteAllText("Concurrency.txt", sw.ToString())
                ()
        ...
        |> withLog
            ...
            (fun logger fs ->
                ...
                let work = async {
                    ...
                    return! ProjectGraphLoader.Execution(bm, graph)
                }
            ...
            (fun logger fs ->
                ...
                Expect.throwsT<TaskCanceledException>
                    (fun () ->
                        ...
                        let build = ProjectGraphLoader.Execution(bm, graph)
...
let debugTests toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    ...
    |> withLog
        ...
        (fun logger fs ->
            ...
            let path = @"C:\Users\jimmy\Repositories\public\TheAngryByrd\FAKE\Fake.sln"
            // let path = @"C:\Users\jimmy\Repositories\public\TheAngryByrd\FAKE\src\app\Fake.Core.Context\Fake.Core.Context.fsproj"

            // let graph = ProjectGraph(path)
            // let sw0 = Stopwatch.StartNew()
            // let factory = workspaceFactory (toolsPath)
            // let res = factory.LoadSln path
            // sw0.Stop()
            let swAll = Stopwatch.StartNew()

            // let graph2 = ProjectGraph(path, ProjectCollection.GlobalProjectCollection)
            let props = ProjectLoader.defaultGlobalProperties
            // |> Map.add "TargetFramework" "net6.0"

            let sw1 = Stopwatch.StartNew()
            let pc = new ProjectCollection(props)
            let graph3 = ProjectGraph(path, pc, (projectInstanceFactoryNeedName selectFirstTfm))
            sw1.Stop()

            let projectsBeforeBuild =
                graph3.ProjectNodes
                |> Array.ofSeq
                |> Array.Parallel.map (fun p ->
                    let p = p.ProjectInstance
                    p.FullPath, ProjectLoader.getLoadedProjectInfo p.FullPath [] (ProjectLoader.LoadedProject p)
                )
            ...
            use sw = new StringWriter()
            // let loggers = ProjectLoader.createLoggers allKnownNames BinaryLogGeneration.Off sw
            let bp = BuildParameters()
            // bp.BuildThreadPriority <- ThreadPriority.Highest
            bp.ProjectLoadSettings <- ProjectLoadSettings.FailOnUnresolvedSdk
            // bp.ProjectLoadSettings <- ProjectLoadSettings.RecordEvaluatedItemElements ||| ProjectLoadSettings.ProfileEvaluation
            bp.LogInitialPropertiesAndItems <- true
            ...
            let result = Async.RunSynchronously(bm.BuildAsync(gbr), cancellationToken = cts.Token)
            sw2.Stop()
            ...
            let projectsAfterBuild =
                ...
                |> Array.Parallel.choose (fun p ->
                    let p = p.Key.ProjectInstance

                    match ProjectLoader.getLoadedProjectInfo p.FullPath [] (ProjectLoader.LoadedProject p) with
                    | Ok v -> Some(p.FullPath, v)
                    | _ -> None
                )

            // let noTfm2 = res |> Seq.toArray |> Array.filter (fun v -> v.TargetFramework = "" || v.PackageReferences.Length = 0)
            let noTfm =
                projectsAfterBuild
                |> Array.filter (fun (k, v) ->
                    v.TargetFramework = ""
                    || v.PackageReferences.Length = 0
                )
